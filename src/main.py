...
def main():
    ...
    grade_pq_pairs(
        ...
        args.result_file_path, model, system_message,args.prompt_mode)

    log_file = Path(args.result_file_path).parent / "logs" / Path(args.result_file_path).name.replace(".txt", ".json")
